		...
		for {
			...
			for openaiStream.Next() {
				...
				for _, choice := range chunk.Choices {
					...
					// DeepSeek / o-series 模型的推理内容（SDK struct 未暴露该字段，手动解析）
					if choice.Delta.Refusal == "" {
						raw := choice.Delta.RawJSON()
						if raw != "" && json.Valid([]byte(raw)) {
							var deltaMap map[string]json.RawMessage
							if err := json.Unmarshal([]byte(raw), &deltaMap); err == nil {
								if rcRaw, ok := deltaMap["reasoning_content"]; ok && len(rcRaw) > 2 {
									rcStr := string(rcRaw)
									rcStr = strings.Trim(rcStr, "\"")
									if rcStr != "" {
										eventChan <- ProviderEvent{
